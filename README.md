...
Набор из трёх Claude Code skill'ов для документирования C#/.NET кода **с выводом на русском языке**.
...
После установки все три skill'а доступны во всех проектах. Вызов:
...
`doc-cs` помечен `disable-model-invocation: true` — массовое редактирование исходников запускается только явной командой `/doc-cs`. `doc-db` безопасен (пишет лишь `database.md`) и может срабатывать автоматически по фразам «опиши БД», «составь database.md».
